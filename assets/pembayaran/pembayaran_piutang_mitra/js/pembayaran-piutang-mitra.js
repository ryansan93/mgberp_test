var ppm = {
    ...
    settingUp: function() {
        ...
        $(div_action).find('select.mitra').select2().on('select2:select', function (e) {
            ppm.getKodePiutang();
        });
    ...
    loadForm: function(id, edit = null, href = null) {
        ...
        $.ajax({
            url : 'pembayaran/PembayaranPiutangMitra/loadForm',
            data : {
                'params' :  params,
                'edit' :  edit
            },
    ...
    getLists: function() {
        ...
        if ( err > 0 ) {
            ...
            $.ajax({
                url :'pembayaran/PembayaranPiutangMitra/getLists',
                dataType: 'html',
				type: 'get',
                data : {
                    'params': data
    ...
    getKodePiutang: function () {
        ...
        if ( !empty(val_mitra) && !empty(val_perusahaan) ) {
            ...
            $.ajax({
                url :'pembayaran/PembayaranPiutangMitra/getKodePiutang',
                dataType: 'json',
				type: 'post',
                data : {
                    'params': params
        ...
        if ( err > 0 ) {
            ...
            bootbox.confirm('Apakah anda yakin ingin menyimpan data ?', function(result) {
                if ( result ) {
                    ...
                    $.ajax({
                        url :'pembayaran/PembayaranPiutangMitra/save',
                        type : 'POST',
                        data : formData,
                        contentType : false,
                        processData : false,
                        ...
                        success : function(data){
                            ...
                            if ( data.status == 1 ) {
                                bootbox.alert(data.message, function(){
                                    ppm.loadForm(data.content.id, null, 'action');
                                });
                            } else {
        ...
        if ( err > 0 ) {
            ...
            bootbox.confirm('Apakah anda yakin ingin meng-ubah data ?', function(result) {
                if ( result ) {
                    ...
                    $.ajax({
                        url :'pembayaran/PembayaranPiutangMitra/edit',
                        type : 'POST',
                        data : formData,
                        contentType : false,
                        processData : false,
    ...
    delete: function(elm) {
        bootbox.confirm('Apakah anda yakin ingin meng-hapus data ?', function(result) {
            if ( result ) {
                ...
                $.ajax({
                    url :'pembayaran/PembayaranPiutangMitra/delete',
                    type : 'POST',
                    dataType : 'JSON',
                    data : {
                        'params': data
