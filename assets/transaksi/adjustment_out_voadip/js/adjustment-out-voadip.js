var aov = {
    ...
    getLists: function() {
        ...
        if ( err > 0 ) {
            ...
            $.ajax({
                url: 'transaksi/AdjustmentOutVoadip/getLists',
                data: { 'params': params },
                type: 'GET',
                dataType: 'HTML',
	...
	loadForm: function(id, edit, href) {
		...
		$.ajax({
            url: 'transaksi/AdjustmentOutVoadip/loadForm',
            data: { 'params': params },
            type: 'GET',
            dataType: 'HTML',
            beforeSend: function(){ showLoading() },
	...
	getSj: function() {
		...
		if ( err > 0 ) {
			...
			$.ajax({
	            url: 'transaksi/AdjustmentOutVoadip/getSj',
	            data: { 'params': params },
	            type: 'POST',
	            dataType: 'JSON',
	            beforeSend: function(){ showLoading() },
		...
		if ( err > 0 ) {
			...
			bootbox.confirm('Apakah anda yakin ingin menyimpan data ?', function (result) {
				if ( result ) {
					...
					$.ajax({
			            url: 'transaksi/AdjustmentOutVoadip/save',
			            data: { 'params': params },
			            type: 'POST',
			            dataType: 'JSON',
			            beforeSend: function(){ showLoading() },
			            success: function(data){
			            	...
			            	if ( data.status == 1 ) {
			            		aov.hitungStokByTransaksi(data.content);
			            		// aov.hitungStokAwal( data.content.id, $(div).find('.no_sj').select2('val') );
	...
	delete: function(elm) {
		...
		bootbox.confirm('Apakah anda yakin ingin meng-hapus data ?', function (result) {
			if ( result ) {
				...
				$.ajax({
					url: 'transaksi/AdjustmentOutVoadip/delete',
					data: { 'params': params },
					type: 'POST',
					dataType: 'JSON',
					beforeSend: function(){ showLoading() },
	...
	hitungStokByTransaksi: function(content) {
		...
		$.ajax({
			url: 'transaksi/AdjustmentOutVoadip/hitungStokByTransaksi',
			data: {
				'params': params
			},
			type: 'POST',
 ...
 //        $.ajax({
 //            url: 'transaksi/AdjustmentOutVoadip/hitungStokAwal',
 //            dataType: 'json',
 //            type: 'post',
 //            data: {
 //                'params': params
 ...
 //                    bootbox.alert(data.message, function() {
 //                    	location.reload();
 //                        // aov.load_form(no_sj, null, 'transaksi');
 //                    });
 //                } else {                    
 //                    bootbox.alert(data.message);
 //                };
