			...
			<tbody class="row-wrapper">
				<?php foreach ($v_item['detail'] as $k_tgl => $v_tgl): ?>
					...
					<?php if ( !empty($v_tgl['masuk']) ): ?>
						...
						<?php endforeach ?>
					<?php endif ?>
					<?php if ( !empty($v_tgl['keluar']) ): ?>
						<?php foreach ($v_tgl['keluar'] as $k_keluar => $v_keluar): ?>
							<?php $saldo -= $v_keluar['keluar']; ?>
							<?php $nilai_saldo -= ($v_keluar['keluar'] * $v_keluar['harga_beli']); ?>
							<tr class="data">
								<?php if ( $idx_item == 0 ): ?>
									<td rowspan="<?php echo $rowspan_item; ?>"><?php echo $v_item['kode']; ?></td>
									<td rowspan="<?php echo $rowspan_item; ?>"><?php echo $v_item['nama']; ?></td>
								...
								<td>Keluar</td>
								<td>
									<?php if ( stristr($v_keluar['jenis_trans'], 'retur') !== false ): ?>
										<b><?php echo $v_keluar['jenis_trans']; ?></b>
										<br>										
									<?php endif ?>
									<?php echo $v_keluar['kode']; ?>
									<hr style="margin-top: 5px; margin-bottom: 5px;">
									<?php echo $v_keluar['ke']; ?>
								</td>
								<td class="text-right"><?php echo angkaDecimal($v_keluar['keluar']); ?></td>
								<td class="text-right"><?php echo angkaDecimalFormat($v_keluar['harga_beli'], $v_keluar['decimal']); ?></td>
								<td class="text-right"><?php echo angkaDecimal($v_keluar['nilai_beli']); ?></td>
								<td class="text-right"><?php echo angkaDecimalFormat($v_keluar['harga_jual'], $v_keluar['decimal']); ?></td>
								<td class="text-right"><?php echo angkaDecimal($v_keluar['nilai_jual']); ?></td>
								<td class="text-right"><?php echo angkaDecimal($saldo); ?></td>
								<td class="text-right"><?php echo angkaDecimal($nilai_saldo); ?></td>
							</tr>
						<?php endforeach ?>
