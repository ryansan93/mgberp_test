<?php if ( !empty($data) && count($data) > 0 ) { ?>
    <?php foreach ($data as $key => $value) { ?>
        <tr class="head cursor-p" onclick="hu.collapseRow(this)">
            <td><?php echo strtoupper(tglIndonesia($value['tanggal'], '-', ' ')); ?></td>
            <td><?php echo strtoupper($value['no_order']); ?></td>
            <td><?php echo strtoupper($value['supplier']); ?></td>
            <td><?php echo strtoupper($value['kode_unit']); ?></td>
            <td class="text-right hutang"><?php echo angkaDecimal($value['hutang']); ?></td>
            ...
            <td colspan="7" style="background-color: #ededed;">
                ...
                <div class="col-xs-12 no-padding">
                    <table class="table table-bordered" style="margin-bottom: 0px;">
                        <thead>
                            <tr>
                                <td class="col-xs-3"><b>Kode Bayar</b></td>
                        ...
                        <tbody>
                            <?php if ( !empty($value['det_bayar']) && count($value['det_bayar']) > 0 ) { ?>
                                <?php foreach ($value['det_bayar'] as $k_det => $v_det) { ?>
                                    <tr>
                                        <td><?php echo strtoupper($v_det['kode_bayar']); ?></td>
