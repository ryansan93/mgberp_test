...
class KirimVoadip extends Public_Controller {
    ...
    public function index($segment=0)
    {
        ...
        if ( $akses['a_view'] == 1 ) {
            $this->add_external_js(array(
                "assets/import/js/kirim-voadip.js"
            ));
            $this->add_external_css(array(
                "assets/import/css/kirim-voadip.css"
            ));
            ...
            // Load Indexx
            $data['title_menu'] = 'Import Kirim Voadip';
            $data['view'] = $this->load->view('import/kirim_voadip/index', $content, TRUE);
                    ...
                    if ( !empty($_data) && $data_tidak_ditemukan == 0 ) {
                        ...
                        foreach ($_data['value'] as $k_val => $val) {
                            ...
                            if ( !isset($data[ $key ]) ) {
                                ...
                                $data[ $key ]['NO SJ'] = isset($val['NO SJ']) ? $val['NO SJ'] : 0;
                        ...
                        if ( !empty($data) && $data_tidak_ditemukan == 0 ) {
                            foreach ($data as $k_data => $v_data) {
                                ...
                                if ( $v_data['JENIS KIRIM'] == 'opks' ) {
                                    ...
                                    $d_ov = $m_ov->where('tanggal', $v_data['TGL KIRIM'])->where('supplier', $v_data['NAMA ASAL'])->where('no_order', 'like', '%'.$v_data['UNIT'].'%')->orderBy('id', 'desc')->first();

                                    $no_order = !empty($d_ov) ? $d_ov->no_order : null;
                                    $no_sj = $v_data['NO SJ'];
                                    $kode_unit = 1;
                                } else {
                            ...
                            if ( count($data_injek) > 0 ) {
                                if ( $jml_row == $jml_data_insert ) {
                                    foreach ($data_injek as $key => $val) {
                                        ...
                                        if ( empty($val['no_order']) && empty($val['no_sj']) ) {
                                        ...
                                        $m_kirim_voadip->tgl_trans = $val['tgl_trans'];
                                        $m_kirim_voadip->tgl_kirim = $val['tgl_kirim'];
                                        $m_kirim_voadip->no_order = $no_order;
                                        $m_kirim_voadip->jenis_kirim = $val['jenis_kirim'];
                                        $m_kirim_voadip->asal = $val['asal'];
                                        $m_kirim_voadip->jenis_tujuan = $val['jenis_tujuan'];
                                        $m_kirim_voadip->tujuan = $val['tujuan'];
                                        $m_kirim_voadip->ekspedisi = $val['ekspedisi'];
                                        $m_kirim_voadip->no_polisi = $val['no_polisi'];
                                        $m_kirim_voadip->sopir = $val['sopir'];
                                        $m_kirim_voadip->no_sj = $no_sj;
                                        $m_kirim_voadip->ongkos_angkut = $val['ongkos_angkut'];
                                        $m_kirim_voadip->save();
                                        ...
                                        foreach ($val['detail'] as $k_detail => $v_detail) {
                                            ...
                                            $m_kirim_voadip_detail->item = $v_detail['item'];
