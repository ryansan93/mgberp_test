...
class TerimaPakan extends Public_Controller {
    ...
    public function index($segment=0)
    {
        ...
        if ( $akses['a_view'] == 1 ) {
            $this->add_external_js(array(
                "assets/import/js/terima-pakan.js"
            ));
            $this->add_external_css(array(
                "assets/import/css/terima-pakan.css"
            ));
            ...
            // Load Indexx
            $data['title_menu'] = 'Import Terima Pakan';
            $data['view'] = $this->load->view('import/terima_pakan/index', $content, TRUE);
                ...
                if ( $moved ) {
                    ...
                    if ( !empty($_data) && $data_tidak_ditemukan == 0 ) {
                        ...
                        foreach ($_data['value'] as $k_val => $val) {
                            $key = $val['NAMA ASAL'].' - '.$val['NAMA TUJUAN'].' - '.str_replace('-', '', $val['TGL KIRIM']).' - '.str_replace('-', '', $val['TGL TERIMA'].' - '.$val['KANDANG'].' - '.$val['NO POLISI']);
                            ...
                            $d_kp = $m_kp->where('tgl_kirim', trim($val['TGL KIRIM']))
                                         ->where('jenis_kirim', trim($data[ $key ]['JENIS KIRIM']))
                                         ->where('asal', trim($data[ $key ]['NAMA ASAL']))
                                         ->where('jenis_tujuan', trim($data[ $key ]['JENIS TUJUAN']))
                                         ->where('tujuan', trim($data[ $key ]['NAMA TUJUAN']))
                                         ->where('ekspedisi', trim($data[ $key ]['EKSPEDISI']))
                                         ->where('no_polisi', 'like', '%'.trim($data[ $key ]['NO POLISI']).'%')
                                         ->where('sopir', trim($data[ $key ]['SOPIR']))
                                         ->where('no_order', 'like', '%'.strtoupper($data[ $key ]['UNIT']).'%')
                                         ->orderBy('id', 'desc')
                                         ->first();
                            ...
                            if ( !$d_kp ) {
                                ...
                                if ( stristr($data[ $key ]['JENIS TUJUAN'], 'gudang') ) {
                                    $m_gdg = new \Model\Storage\Gudang_model();
                                    $d_gdg = $m_gdg->where('id', $data[ $key ]['NAMA TUJUAN'])->where('jenis', 'PAKAN')->orderBy('id', 'desc')->first();
                                ...
                                if ( stristr($data[ $key ]['JENIS KIRIM'], 'opkg') !== FALSE ) {
                                    $m_gdg = new \Model\Storage\Gudang_model();
                                    $d_gdg = $m_gdg->where('id', $data[ $key ]['NAMA ASAL'])->where('jenis', 'PAKAN')->orderBy('id', 'desc')->first();
                                ...
                                cetak_r('DATA PENGIRIMAN DENGAN TUJUAN : '.trim(strtoupper($data[ $key ]['JENIS TUJUAN'])).'('.$tujuan.') DARI : '.trim(strtoupper($data[ $key ]['NAMA ASAL'])).'('.$asal.') tidak ditemukan.', 1);

                                break;
                            } else {
                                $id_kirim_pakan = $d_kp->id;
                            }

                            $data[ $key ]['ID KIRIM PAKAN'] = $id_kirim_pakan;
                        }
                        ...
                        if ( !empty($data) ) {
                            foreach ($data as $k_data => $v_data) {
                                ...
                                $m_terima_pakan->id_kirim_pakan = $v_data['ID KIRIM PAKAN'];
                                $m_terima_pakan->tgl_trans = $now['waktu'];
                                $m_terima_pakan->tgl_terima = $v_data['TGL TERIMA'];
                                $m_terima_pakan->save();
                                ...
                                foreach ($v_data['DETAIL'] as $k_detail => $v_detail) {
                                    ...
                                    $m_terima_pakan_detail->item = $v_detail['NAMA ITEM'];
