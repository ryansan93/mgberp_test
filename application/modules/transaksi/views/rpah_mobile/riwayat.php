<form class="form-horizontal">
				...
				<select id="select_unit" class="form-control selectpicker" data-live-search="true" type="text" data-required="1">
					...
					<?php foreach ($unit as $k_unit => $v_unit): ?>
						<?php 
							$selected = null;
							if ( !empty($kode_unit) ) {
								if ( $v_unit['kode'] == $kode_unit ) {
									$selected = 'selected';
								}
							}
						?>
						<option data-tokens="<?php echo $v_unit['nama']; ?>" value="<?php echo $v_unit['kode']; ?>" <?php echo $selected; ?> ><?php echo strtoupper($v_unit['nama']); ?></option>
					<?php endforeach ?>
				</select>
			</div>
		</div>
		...
		<div class="col-xs-12 no-padding">
			...
			<div class="col-xs-8 no-padding">
				<div class="input-group date datetimepicker" name="tanggal" id="tanggal">
	                <input type="text" class="form-control text-center uppercase" placeholder="Tanggal" data-required="1" data-tgl="<?php echo $tgl_panen; ?>" />
	...
	<div class="col-xs-12 no-padding" style="margin-top: 5px; margin-bottom: 5px;">
		<button id="btn-get-lists" type="button" class="btn btn-primary cursor-p col-xs-12" title="ADD" onclick="rm.get_lists()"> 
			<i class="fa fa-search" aria-hidden="true"></i> Tampilkan
		</button>
	</div>
	<div class="col-xs-12 no-padding">
		<button id="btn-add" type="button" data-href="transaksi" class="btn btn-primary cursor-p col-xs-12" title="ADD" onclick="rm.changeTabActive(this)"> 
...
<small>
	...
	<table class="table table-bordered tbl_riwayat" style="margin-bottom: 0px;">
		<thead>
			<tr>
				<th class="col-xs-4">Mitra</th>
				<th class="col-xs-3">Ekor Panen</th>
				<th class="col-xs-3">Kg Panen</th>
				<th class="col-xs-1">BB Panen</th>
			</tr>
		</thead>
		<tbody>
			<tr>
