        ...
        <div class="form-group">
            <small>
                <table class="table table-bordered tbl_data_op header">
                    <thead>
                        <tr>
                            <th class="col-xs-2 text-center">Nama Item</th>
                            <th class="col-xs-1 text-center">Jumlah (Kg)</th>
                            <th class="col-xs-1 text-center">Jumlah Retur (Kg)</th>
                            <th class="col-xs-2 text-center">Kondisi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if ( count($data['detail']) > 0) : ?>
                            <?php foreach ($data['detail'] as $k_det => $v_det): ?>
                                <tr class="v-center">
                                    ...
                                    <td class="text-right"><?php echo angkaRibuan($v_det['jumlah_rp']); ?></td>
...
<div class="col-xs-12 no-padding">
    ...
    <?php if ( $akses['a_delete'] == 1 ): ?>
        <button type="button" class="btn btn-danger pull-right" onclick="rp.delete(this)" data-href="riwayat_rp" data-id="<?php echo $data['id']; ?>" style="margin-right: 10px;"><i class="fa fa-trash"></i> Hapus</button>
    <?php endif ?>
</div>