			...
			<?php foreach ($rdim_data_perwakilan_mitra as $key => $perwakilan): ?>
				...
				<?php if ($perwakilan['child']): ?>
					...
					<?php 
						$index = 0;
						do { ?>
							<?php // if ( isset($data_details[$_key][$index]) ) { ?>
								<?php 
								...
								<tr class="child v-center <?php echo $inactive; ?>" data-key="<?php echo $_key ?>">
									...
									<td class="page0">
										<select class="form-control" name="mitra" onchange="rdim.changeMitraRow(this)" >
											...
											<?php foreach ($perwakilan['child'] as $key => $mitra): ?>
												...
												?>
												<option value="<?php echo $mitra['mitra_id'] ?>" data-jenis="<?php echo $mitra['jenis'] ?>" data-kandangs='<?php echo json_encode($mitra['kandangs']) ?>' <?php echo $selected; ?> ><?php echo $mitra['kode_perusahaan'].' | '.$mitra['nama'].' '.$unit; ?></option>
											<?php endforeach; ?>
										</select>
									</td>
									...
									<td class="page1 hide"><input class="form-control no-check hide" type="text" name="istirahat_kandang" value="" data-tipe="integer" readonly></td>
									...
									<td class="page2">
										<select class="form-control" name="pengawas">
											...
											<?php foreach ($perwakilan['parent']['kanit'] as $k_kanit => $v_kanit): ?>
												<?php
													$selected = null;
													if ( $v_kanit['nik'] == $data_details[$_key][$index]['nik_pengawas'] ) {
														$selected = 'selected';
													}
												?>
												<option value="<?php echo $v_kanit['nik']; ?>" <?php echo $selected; ?> ><?php echo $v_kanit['nama']; ?></option>
											<?php endforeach ?>
										</select>
									...
									<td class="page2">
										<select class="form-control" name="tim_sampling">
											...
											<?php foreach ($perwakilan['parent']['ppl'] as $k_ppl => $v_ppl): ?>
												<?php
													$selected = null;
													if ( $v_ppl['nik'] == $data_details[$_key][$index]['nik_sampling'] ) {
														$selected = 'selected';
													}
												?>
												<option value="<?php echo $v_ppl['nik']; ?>" <?php echo $selected; ?> ><?php echo $v_ppl['nama']; ?></option>
											<?php endforeach ?>
										</select>
									...
									<td class="page2">
										<select class="form-control" name="tim_panen">
											...
											<?php foreach ($perwakilan['parent']['marketing'] as $k_marketing => $v_marketing): ?>
												<?php
													$selected = null;
													if ( $v_marketing['nik'] == $data_details[$_key][$index]['nik_tim_panen'] ) {
														$selected = 'selected';
													}
												?>
												<option value="<?php echo $v_marketing['nik']; ?>" <?php echo $selected; ?> ><?php echo $v_marketing['nama']; ?></option>
											<?php endforeach ?>
										</select>
										<!-- <select class="form-control" name="tim_panen">
									...
									<td class="page2">
										<select class="form-control" name="koordinator_area">
											...
											<?php foreach ($perwakilan['parent']['koordinator'] as $k_koordinator => $v_koordinator): ?>
												<?php
													$selected = null;
													if ( $v_koordinator['nik'] == $data_details[$_key][$index]['nik_koar'] ) {
														$selected = 'selected';
													}
												?>
												<option value="<?php echo $v_koordinator['nik']; ?>" <?php echo $selected; ?> ><?php echo $v_koordinator['nama']; ?></option>
											<?php endforeach ?>
										</select>
									...
									<td class="page2">
										<select class="form-control" name="formatPb" data-f="<?php echo $data_details[$_key][$index]['format_pb']; ?>" data-nomor="<?php echo trim($data_details[$_key][$index]['nomor_sk']); ?>">
											<option value="">pilih</option>
											<?php foreach ($perwakilan['parent']['formatPb'] as $format): ?>
												<?php 
													$selected = null;
													if ( isset($data_details[$_key][$index]['format_pb']) ) {
														if ( trim($format['nomor']) == trim($data_details[$_key][$index]['nomor_sk']) ) {
															// cetak_r( $data_details[$_key][$index]['perusahaan'].' | '.$format['perusahaan'] );
												...
												?>
												<option value="<?php echo $format['id'] ?>" data-perusahaan="<?php echo $format['perusahaan']; ?>" data-pola="<?php echo $format['pola']; ?>" data-nomor="<?php echo trim($format['nomor']); ?>" <?php echo $selected; ?> ><?php echo tglIndonesia($format['tgl_berlaku'], '-', ' ').' - '.$format['format']; ?></option>
											<?php endforeach; ?>
										</select>
										<div class="btn-ctrl" <?php // echo $hide; ?> >
											<span onclick="rdim.removeRowChild(this)" class="btn_del_row_2x"></span>
									...
									<td class="page2 hide"><input class="form-control" type="text" name="jenis" value="<?php echo !isset($data_details[$_key][$index]['pola']) ?: $data_details[$_key][$index]['pola']; ?>" data-tipe="text" readonly></td>
									<td class="page2 hide">
