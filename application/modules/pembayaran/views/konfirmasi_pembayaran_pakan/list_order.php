<?php if ( !empty($data) && count($data) > 0 ): ?>
	<?php foreach ($data as $k_data => $v_data): ?>
		<tr class="cursor-p header" title="Klik untuk melihat detail" data-supplier="<?php echo $v_data['kode_supplier']; ?>">
			...
			<td class="text-right jumlah" data-val="<?php echo $v_data['jumlah']; ?>"><?php echo angkaRibuan($v_data['jumlah']); ?></td>
				...
				<table class="table table-bordered" style="margin-bottom: 0px;">
					<thead>
						<tr>
							...
							<th class="col-xs-3" style="background-color: #adb3ff;">Jenis Pakan</th>
					...
					<tbody>
						<?php foreach ($v_data['detail'] as $k_det => $v_det): ?>
							<tr>
								...
								<td class="text-right jumlah" data-val="<?php echo $v_det['jumlah']; ?>"><?php echo angkaRibuan($v_det['jumlah']); ?></td>
