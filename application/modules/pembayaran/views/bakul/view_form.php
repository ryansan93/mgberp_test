				...
				<?php foreach ($data['detail'] as $k_det => $v_det): ?>
					<tr class="data" data-id="<?php echo $v_det['id_do']; ?>">
						...
						<td class="text-center status">
							<?php
								$ket = '';
								$sudah_bayar = isset($v_det['sudah_bayar']) ? round($v_det['sudah_bayar'], 2): 0;
								$total_bayar = round(($v_det['jumlah_bayar']+$sudah_bayar) + $v_det['penyesuaian'], 2);

								if ( round($total, 2) <= $total_bayar ) {
									$ket = '<span style="color: blue;"><b>LUNAS</b></span>';
								} else if ( round($total, 2) > $sudah_bayar ) {
