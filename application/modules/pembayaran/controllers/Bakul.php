...
class Bakul extends Public_Controller
{
    ...
    public function view_form($id)
    {
    	...
    	if ( $d_pp ) {
    		...
    		$d_pp_next = $_m_pp->select('id')->where('no_pelanggan', $d_pp['no_pelanggan'])->where('perusahaan', $d_pp['perusahaan']['kode'])->where('id', '>', $id)->where('tgl_bayar', '>', $d_pp['tgl_bayar'])->get();
            ...
            $edit = 1;
            if ( $d_pp_next->count() > 0 ) {
                $edit = 0;
                // cetak_r( $d_pp['id'] );
                // cetak_r( $d_pp_next->toArray() );
            }
