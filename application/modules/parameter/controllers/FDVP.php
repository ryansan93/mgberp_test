    ...
    public function getList($tipe = null) {
        ...
        $sql = "
            ...
            left join
                ...
                on
                    brg.kode_supplier = supl_not_pakan.nomor
            left join
                (
                    select 
                        sp.*,
                        supl.nama
                    from supplier_pakan sp
                    right join
                        (
                            select plg1.* from pelanggan plg1
                            right join
                        ...
                        on
                            sp.id_supl = supl.nomor
                ) supl_pakan
                on
                    brg.id = supl_pakan.id_pakan
            left join
                (
                    select lt1.* from log_tables lt1
                    right join
                        (select max(id) as id, tbl_id from log_tables where tbl_name = 'barang' group by tbl_id) lt2
                        on
                            lt1.id = lt2.id
                ) lt
                on
